...
static int rx_enable(void)
{
	...
	atomic_set_bit(&uart_state, SM_RX_ENABLED_BIT);
...
static int rx_disable(void)
{
	...
	atomic_set_bit(&uart_state, SM_RX_RECOVERY_DISABLED_BIT);

	while (atomic_test_bit(&uart_state, SM_RX_RECOVERY_BIT)) {
...
static void rx_recovery(void)
{
	...
	if (atomic_test_bit(&uart_state, SM_RX_RECOVERY_DISABLED_BIT)) {
		return;
	}

	atomic_set_bit(&uart_state, SM_RX_RECOVERY_BIT);
	...
	atomic_clear_bit(&uart_state, SM_RX_RECOVERY_BIT);
}
...
static void rx_process(struct k_work *work)
{
	...
	if (atomic_test_bit(&sm_pipe.state, SM_PIPE_INIT_BIT)) {
		if (atomic_test_bit(&sm_pipe.state, SM_PIPE_OPEN_BIT)) {
...
{
	if (!atomic_test_and_set_bit(&uart_state, SM_TX_ENABLED_BIT)) {
...
static int tx_disable(k_timeout_t timeout)
{
	...
	if (!atomic_test_and_clear_bit(&uart_state, SM_TX_ENABLED_BIT)) {
		return 0;
	}
...
static int tx_start(void)
{
	...
	if (!atomic_test_bit(&uart_state, SM_TX_ENABLED_BIT)) {
		return -EAGAIN;
	}
...
#if SM_PIPE
	if (atomic_test_bit(&sm_pipe.state, SM_PIPE_INIT_BIT) &&
	    !atomic_test_bit(&sm_pipe.state, SM_PIPE_OPEN_BIT) &&
	    !atomic_test_bit(&uart_state, SM_RX_ENABLED_BIT) &&
	    !atomic_test_bit(&uart_state, SM_TX_ENABLED_BIT)) {
...
{
#if SM_PIPE
	if (atomic_test_bit(&sm_pipe.state, SM_PIPE_INIT_BIT) && sm_pipe.tx_cb != NULL) {
...
static int pipe_open(void *data)
{
	...
	atomic_clear_bit(&uart_state, SM_RX_RECOVERY_DISABLED_BIT);
	ret = rx_enable();
	if (ret) {
		return ret;
	}
	...
	atomic_set_bit(&sm_pipe.state, SM_PIPE_OPEN_BIT);
...
static int pipe_close(void *data)
{
	...
	atomic_clear_bit(&sm_pipe.state, SM_PIPE_OPEN_BIT);
...
struct modem_pipe *sm_uart_pipe_init(sm_pipe_tx_t pipe_tx_cb)
{
	...
	atomic_set_bit(&sm_pipe.state, SM_PIPE_INIT_BIT);
