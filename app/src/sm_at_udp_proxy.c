...
static int do_udp_proxy_close(void)
{
	...
	ret = k_thread_join(&udp_thread, K_SECONDS(CONFIG_SM_UDP_POLL_TIME));
	if (ret) {
...
static void udp_thread_func(void *p1, void *p2, void *p3)
{
	...
	do {
		ret = zsock_poll(fds, ARRAY_SIZE(fds), MSEC_PER_SEC * CONFIG_SM_UDP_POLL_TIME);
		if (ret < 0) {  /* IO error */
...
static bool socket_is_in_use(void)
{
	...
	LOG_ERR("The UDP socket is already in use by the client. Disconnect it first.");
	return true;
}
