...
static bool handle_bootloader_at_cmd(uint8_t *buf, size_t buf_size, char *at_cmd)
{
	int err;

	if (strncasecmp(at_cmd, AT_XDFU_WRITE_CMD,
...
static void cmd_send(uint8_t *buf, size_t cmd_length, size_t buf_size, bool *stop_at_receive)
{
	...
	if (sm_bootloader_mode_enabled) {
		if (handle_bootloader_at_cmd(buf, buf_size, at_cmd)) {
			return;
		}
	} else {
