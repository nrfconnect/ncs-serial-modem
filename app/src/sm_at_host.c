...
#define AT_XREBOOT_CMD "AT#XREBOOT"
...
static void cmd_send(uint8_t *buf, size_t cmd_length, size_t buf_size, bool *stop_at_receive)
{
	...
	if (sm_bootloader_mode_enabled) {
		...
		} else if (strncasecmp(at_cmd, AT_XREBOOT_CMD, sizeof(AT_XREBOOT_CMD) - 1) == 0) {
			LOG_INF("Rebooting device via %s command", AT_XREBOOT_CMD);
			LOG_PANIC();
