...
static void fota_dl_handler(const struct fota_download_evt *evt)
{
	...
	case FOTA_DOWNLOAD_EVT_FINISHED:
		...
		sm_fota_bl_pending_validate = (sm_fota_type == SM_FOTA_TYPE_MCUBOOT_B1);
#endif
			  ...
			  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
...
#endif
		{
			...
			if (op == SM_FOTA_START_APP || op == SM_FOTA_START_MCUBOOT_B1) {
...
#endif
#if defined(PM_S1_ADDRESS)
			else if (op == SM_FOTA_START_MCUBOOT_B1) {
				sm_fota_type = SM_FOTA_TYPE_MCUBOOT_B1;

				/* Save the B1 version before FOTA. */
				sm_util_b1_active_version(&sm_fota_bl_version_before);
				sm_settings_fota_save();
			}
#endif
			else {
	...
	case AT_PARSER_CMD_TYPE_TEST:
		...
		rsp_send("\r\n#XFOTA: "
			 ...
			 SM_FOTA_START_FULL_FOTA, SM_FOTA_START_MCUBOOT_B1, SM_FOTA_MFW_READ,
			 SM_FOTA_ERASE_MFW);
#elif defined(PM_S1_ADDRESS)
		rsp_send("\r\n#XFOTA: (%d,%d,%d,%d,%d,%d)[,<file_url>[,<sec_tag>[,<pdn_id>]]]\r\n",
			 SM_FOTA_STOP, SM_FOTA_START_APP, SM_FOTA_START_MFW,
			 SM_FOTA_START_MCUBOOT_B1, SM_FOTA_MFW_READ, SM_FOTA_ERASE_MFW);
...
void sm_fota_mcuboot_bl_boot_check(void)
{
	...
	int err = sm_util_b1_active_version(&current_version);

	if (err != 0) {
		sm_fota_status = FOTA_STATUS_ERROR;
		sm_fota_info = err;
		LOG_ERR("B1 validate: FAIL (fw_info read err %d)", err);
	} else if (current_version <= sm_fota_bl_version_before) {
		sm_fota_status = FOTA_STATUS_ERROR;
		sm_fota_info = 1;
		LOG_WRN("B1 validate: FAIL (version unchanged: %u -> %u)",
...
{
#if defined(CONFIG_LWM2M_CARRIER)
	if ((sm_fota_type == SM_FOTA_TYPE_APP || sm_fota_type == SM_FOTA_TYPE_MCUBOOT_B1) &&
	    (sm_fota_status == FOTA_STATUS_OK) &&
	    (sm_fota_stage == FOTA_STAGE_COMPLETE)) {
