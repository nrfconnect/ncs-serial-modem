...
/**@brief Socket auto receive flags. */
enum sm_socket_auto_recv_flags {
	AT_SOCKET_AUTO_RECV_DISABLE = 0,         /* Disable auto receive */
	AT_SOCKET_AUTO_RECV_ENABLE_AT_MODE = 1,  /* Enable auto receive in AT mode */
	AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE = 2 /* Enable auto receive in data mode */
};
...
static void auto_recv(struct sm_socket *sock)
{
	int err = 0;
	...
	if (sock->type == NRF_SOCK_STREAM) {
		err = do_recv(sock, 0, NRF_MSG_DONTWAIT,
			      sock->async_poll.auto_recv_hex ? AT_SOCKET_MODE_HEX
...
static void poll_work_fn(struct k_work *work)
{
	...
	for (int i = 0; i < SM_MAX_SOCKET_COUNT; i++) {
		...
		revents = atomic_clear(&socks[i].async_poll.revents);
		LOG_DBG("Socket %d poll revents 0x%x", socks[i].fd, revents);
		if (!at_mode) {
			/* Store events for later processing. */
			socks[i].async_poll.delayed_revents |= revents;
			LOG_DBG("Socket %d delayed revents 0x%x", socks[i].fd,
				socks[i].async_poll.delayed_revents);
		}
		if (!at_mode && !data_mode) {
			/* Do not process any socket events. */
			continue;
		}
		if (data_mode && &socks[i] != datamode_sock) {
			/* Only process events for data mode socket for auto receive. */
			continue;
		}
		if (at_mode && socks[i].async_poll.delayed_revents) {
			...
			update_poll_events(&socks[i], socks[i].async_poll.delayed_revents | revents,
					   true);
			socks[i].async_poll.delayed_revents = 0;
			continue;
		}
		assert(at_mode || (data_mode && &socks[i] == datamode_sock));

		/* Send URC for poll event */
		if (!data_mode && poll_ctx.xapoll_running) {
			uint16_t xapoll_events = revents & (socks[i].async_poll.xapoll_events);

			/* Do not send URC for the same events twice, unless send/recv is done. */
			socks[i].async_poll.xapoll_events &= ~xapoll_events;
			if (xapoll_events) {
				rsp_send("\r\n#XAPOLL: %d,%d\r\n", socks[i].fd, xapoll_events);
			}
		}

		if (revents & NRF_POLLOUT) {
			/* Remove POLLOUT from poll, until send is done. */
			socks[i].async_poll.events &= ~NRF_POLLOUT;
		}
		...
		if (revents & NRF_POLLIN) {
			if (((at_mode && (socks[i].async_poll.auto_recv_flags &
					  AT_SOCKET_AUTO_RECV_ENABLE_AT_MODE)) ||
			     (data_mode && (socks[i].async_poll.auto_recv_flags &
					    AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE)))) {
				/* Automatic receive, keep POLLIN */
				auto_recv(&socks[i]);
			} else {
				/* Remove POLLIN from poll, until recv is done. */
				socks[i].async_poll.events &= ~NRF_POLLIN;
			}
		}

		LOG_DBG("Socket %d, revents %d, disable %d, events %d, xapoll_events %d",
			socks[i].fd, revents, socks[i].async_poll.disable,
			socks[i].async_poll.events, socks[i].async_poll.xapoll_events);

		/* Re-register for remaining events */
		update_poll_events(&socks[i], 0, false);
			  ...
			  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if (mode == AT_SOCKET_MODE_UNFORMATTED || mode == AT_SOCKET_MODE_HEX) {
			...
			if (!err && sock->async_poll.auto_recv_flags &
					    AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE) {
...
SM_AT_CMD_CUSTOM(xsocketmode, "AT#XSOCKETMODE", handle_at_socketmode);
static int handle_at_socketmode(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
				 uint32_t param_count)
{
	int err = -EINVAL;
	int fd;
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		if ((flags & AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE) && hex_mode) {
			/* Not supported. */
			return -EINVAL;
		}
		if (flags) {
		...
		if(!err) {
			sock->async_poll.auto_recv_flags = flags;
			sock->async_poll.auto_recv_hex = hex_mode != 0;
		}
		break;

	case AT_PARSER_CMD_TYPE_TEST:
		rsp_send("\r\n#XSOCKETMODE: <handle>,(%d,%d,%d,%d),(%d,%d)\r\n",
			 AT_SOCKET_AUTO_RECV_DISABLE, AT_SOCKET_AUTO_RECV_ENABLE_AT_MODE,
			 AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE,
			 AT_SOCKET_AUTO_RECV_ENABLE_AT_MODE | AT_SOCKET_AUTO_RECV_ENABLE_DATA_MODE,
