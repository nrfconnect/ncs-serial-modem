	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		case DFU_TYPE_MCUBOOT_B1:
			...
			LOG_INF("%s DFU initialized successfully",
				type == DFU_TYPE_APP ? "MCUboot" : "MCUboot B1");
			return 0;
		case DFU_TYPE_DELTA_MFW:
			err = at_parser_num_get(parser, 2, &size);
			if (err) {
...
#if defined(PM_S1_ADDRESS)
		rsp_send("\r\n#XDFUINIT: (%d,%d,%d),<size>\r\n",
			DFU_TYPE_APP, DFU_TYPE_DELTA_MFW, DFU_TYPE_MCUBOOT_B1);
#else
		rsp_send("\r\n#XDFUINIT: (%d,%d),<size>\r\n",
		...
		case DFU_TYPE_MCUBOOT_B1:
			...
			if (err) {
				LOG_ERR("%s update finalize failed: %d",
					type == DFU_TYPE_APP ? "App firmware" : "MCUboot B1", err);
			} else {
			...
			if (!err) {
				...
				type == DFU_TYPE_APP ? "App firmware" : "MCUboot B1");
			}
