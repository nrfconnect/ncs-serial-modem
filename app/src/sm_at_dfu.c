...
static int xdfu_datamode_callback(uint8_t op, const uint8_t *data, int len, uint8_t flags)
{
	...
	case DATAMODE_EXIT: {
		...
		urc_send("#XDFU:%u,%u,%d\r\n",
			xdfu_current_image_type, DFU_OPERATION_DATA_WRITE, xdfu_status);
				  ...
				  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		case DFU_TYPE_DELTA_MFW:
			...
			return 0;
		case DFU_TYPE_FULL_MFW:
			if (!IS_ENABLED(CONFIG_SM_FULL_MODEM_DFU)) {
...
#if defined(CONFIG_SM_FULL_MODEM_DFU)
		rsp_send("\r\n#XDFUINIT: (%d,%d),<size>\r\n#XDFUINIT: (%d)\r\n",
				  ...
				  uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET:
		...
		case DFU_TYPE_APP:
			...
			urc_send("#XDFU:%u,%u,%d\r\n",
				DFU_TYPE_APP, DFU_OPERATION_APPLY_UPDATE, err);
		...
		case DFU_TYPE_DELTA_MFW:
			...
			urc_send("#XDFU:%u,%u,%d\r\n",
				DFU_TYPE_DELTA_MFW, DFU_OPERATION_APPLY_UPDATE, err);
		...
		case DFU_TYPE_FULL_MFW:
			...
			urc_send("#XDFU:%u,%u,%d\r\n",
				DFU_TYPE_FULL_MFW, DFU_OPERATION_APPLY_UPDATE, err);
