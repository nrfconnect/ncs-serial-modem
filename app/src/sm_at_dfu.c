/*
 * Copyright (c) 2025 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 */

#include <stdbool.h>
#include <stdint.h>
#include <errno.h>
#include <zephyr/kernel.h>
#include <zephyr/sys/reboot.h>
#include <zephyr/logging/log.h>
#include <zephyr/logging/log_ctrl.h>
#include <modem/nrf_modem_lib.h>
#include <nrf_modem_bootloader.h>
#include <dfu/dfu_target.h>
#include <dfu/dfu_target_modem_delta.h>
#include <dfu/dfu_target_mcuboot.h>
#include "sm_at_host.h"
#include "sm_at_dfu.h"
#include "sm_settings.h"
#include "sm_uart_handler.h"

LOG_MODULE_REGISTER(sm_dfu, CONFIG_SM_LOG_LEVEL);

#define APP_DFU_BUFFER_SIZE CONFIG_SM_DATAMODE_BUF_SIZE

bool sm_bootloader_mode_requested;
bool sm_bootloader_mode_enabled;

enum xdfu_image_type {
	DFU_TYPE_APP = 0,
	DFU_TYPE_DELTA_MFW = 1,
	DFU_TYPE_FULL_MFW = 2,
};

enum xdfu_full_mfw_segment_type {
	DFU_FULL_MFW_SEGMENT_BOOTLOADER = 0,
	DFU_FULL_MFW_SEGMENT_FIRMWARE = 1,
};

enum xdfu_operation {
	DFU_OPERATION_INITIALIZE = 0,
	DFU_OPERATION_DATA_WRITE = 1,
	DFU_OPERATION_APPLY_UPDATE = 2,
};
struct xdfu_app_datamode_context {
	size_t addr;
	size_t len;
} xdfu_app_datamode_context;

struct xdfu_delta_mfw_datamode_context {
	size_t addr;
	size_t len;
} xdfu_delta_mfw_datamode_context;

struct xdfu_full_mfw_datamode_context {
	uint16_t type;
	uint32_t addr;
	uint32_t len;
} xdfu_full_mfw_datamode_context;

static uint8_t app_dfu_buffer[APP_DFU_BUFFER_SIZE] __aligned(4);
static bool app_dfu_buffer_initialized;

static uint32_t xdfu_bytes_written;
static int xdfu_status;

static void delta_dfu_evt_handler(enum dfu_target_evt_id evt_id)
{
	switch (evt_id) {
	case DFU_TARGET_EVT_ERASE_PENDING:
		LOG_INF("Delta DFU erase pending");
		break;
	case DFU_TARGET_EVT_TIMEOUT:
		LOG_WRN("Delta DFU erase timeout");
		break;
	case DFU_TARGET_EVT_ERASE_DONE:
		LOG_INF("Delta DFU erase done");
		break;
	default:
		break;
	}
}

int request_bootloader_mode(bool enable)
{
	int err;

	sm_bootloader_mode_requested = enable;

	err = sm_settings_bootloader_mode_save();
	if (err) {
		LOG_ERR("Failed to set bootloader mode requested to: %s",
			enable ? "enabled" : "disabled");
		return err;
	}

	LOG_DBG("Bootloader mode request set to: %s", enable ? "enabled" : "disabled");

	return 0;
}

static int xdfu_app_datamode_callback(uint8_t op, const uint8_t *data, int len, uint8_t flags)
{
	int err;

	switch (op) {
	case DATAMODE_SEND:
		if (data == NULL || len <= 0) {
			LOG_ERR("Chunk data invalid (data=%p len=%d)", (void *)data, len);
			xdfu_status = -EINVAL;
			return -EINVAL;
		}

		err = dfu_target_mcuboot_write((const void *)data, len);
		if (err) {
			LOG_ERR("Failed to write app firmware: %d", err);
			xdfu_status = err;
			return 0;
		}

		xdfu_bytes_written += len;

		return 0;
	case DATAMODE_EXIT:
		if (xdfu_status == 0 && xdfu_bytes_written != xdfu_app_datamode_context.len) {
			LOG_WRN("Wrote %u bytes, expected %u", xdfu_bytes_written,
				xdfu_app_datamode_context.len);
			xdfu_status = -EIO;
		}

		urc_send("#XDFU:%u,%u,%d\r\n", DFU_TYPE_APP, DFU_OPERATION_DATA_WRITE, xdfu_status);

		/* Reset for next chunk. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;

		return 0;
	default:
		LOG_WRN("Unexpected datamode op: %u (flags=0x%02x)", op, flags);
		return 0;
	}
}

static int xdfu_delta_mfw_datamode_callback(uint8_t op, const uint8_t *data, int len, uint8_t flags)
{
	int err;

	switch (op) {
	case DATAMODE_SEND:
		if (data == NULL || len <= 0) {
			LOG_ERR("Chunk data invalid (data=%p len=%d)", (void *)data, len);
			xdfu_status = -EINVAL;
			return -EINVAL;
		}

		err = dfu_target_modem_delta_write((const void *)data, len);
		if (err) {
			LOG_ERR("Failed to write delta modem firmware: %d", err);
			xdfu_status = err;
			return 0;
		}

		xdfu_bytes_written += len;

		return 0;
	case DATAMODE_EXIT:
		if (xdfu_status == 0 && xdfu_bytes_written != xdfu_delta_mfw_datamode_context.len) {
			LOG_WRN("Wrote %u bytes, expected %u", xdfu_bytes_written,
				xdfu_delta_mfw_datamode_context.len);
			xdfu_status = -EIO;
		}

		urc_send("#XDFU:%u,%u,%d\r\n",
			 DFU_TYPE_DELTA_MFW, DFU_OPERATION_DATA_WRITE, xdfu_status);

		/* Reset for next chunk. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;

		return 0;
	default:
		LOG_WRN("Unexpected datamode op: %u (flags=0x%02x)", op, flags);
		return 0;
	}
}

static int xdfu_full_mfw_datamode_callback(uint8_t op, const uint8_t *data, int len, uint8_t flags)
{
	int err;

	switch (op) {
	case DATAMODE_SEND:
		if (data == NULL || len <= 0) {
			LOG_ERR("Chunk data invalid (data=%p len=%d)", (void *)data, len);
			xdfu_status = -EINVAL;
			return -EINVAL;
		}

		if (xdfu_full_mfw_datamode_context.type == DFU_FULL_MFW_SEGMENT_BOOTLOADER) {
			err = nrf_modem_bootloader_bl_write((void *)data, len);
			if (err) {
				LOG_ERR("Failed to write bootloader segment: %d", err);
				xdfu_status = err;
				return 0;
			}
		} else if (xdfu_full_mfw_datamode_context.type == DFU_FULL_MFW_SEGMENT_FIRMWARE) {
			err = nrf_modem_bootloader_fw_write(xdfu_full_mfw_datamode_context.addr,
							    (void *)data, len);
			if (err) {
				LOG_ERR("Failed to write firmware segment: %d", err);
				xdfu_status = err;
				return 0;
			}
		} else {
			LOG_ERR("Invalid segment type: %d", xdfu_full_mfw_datamode_context.type);
			xdfu_status = -EINVAL;
			return -EINVAL;
		}

		xdfu_bytes_written += len;

		return 0;

	case DATAMODE_EXIT: {
		if (xdfu_status == 0 && xdfu_bytes_written != xdfu_full_mfw_datamode_context.len) {
			LOG_WRN("Wrote %u bytes, expected %u", xdfu_bytes_written,
				xdfu_full_mfw_datamode_context.len);
			xdfu_status = -EIO;
		}

		urc_send("#XDFU:%u,%u,%d\r\n",
			 DFU_TYPE_FULL_MFW, DFU_OPERATION_DATA_WRITE, xdfu_status);

		/* Reset for next chunk. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;
		return 0;
	}

	default:
		LOG_WRN("Unexpected datamode op: %u (flags=0x%02x)", op, flags);
		return 0;
	}
}

static int handle_at_xdfu_app(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
				  uint32_t param_count)
{
	int err;
	uint16_t op;

	err = at_parser_num_get(parser, 2, &op);
	if (err) {
		return err;
	}

	switch (op) {
	case DFU_OPERATION_INITIALIZE:
		size_t dfu_size;

		err = at_parser_num_get(parser, 3, &dfu_size);
		if (err) {
			LOG_ERR("Failed to get size: %d", err);
			return err;
		}

		if (!app_dfu_buffer_initialized) {
			err = dfu_target_mcuboot_set_buf(app_dfu_buffer, APP_DFU_BUFFER_SIZE);
			if (err) {
				LOG_ERR("Failed to set app firmware buffer: %d", err);
				return err;
			}
			app_dfu_buffer_initialized = true;
		}

		err = dfu_target_mcuboot_init(dfu_size, 0, NULL);
		if (err) {
			LOG_ERR("Failed to initialize MCUBoot DFU target: %d", err);
			return err;
		}

		return 0;
	case DFU_OPERATION_DATA_WRITE:
		if (param_count != 5) {
			LOG_ERR("Invalid number of parameters for data write");
			return -EINVAL;
		}
		err = at_parser_num_get(parser, 3, &xdfu_app_datamode_context.addr);
		if (err) {
			LOG_ERR("Failed to get address: %d", err);
			return err;
		}
		err = at_parser_num_get(parser, 4, &xdfu_app_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to get length: %d", err);
			return err;
		}

		if (xdfu_app_datamode_context.len == 0) {
			LOG_ERR("Length cannot be 0");
			return -EINVAL;
		}

		/* Prepare per-chunk accounting for the datamode callback. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;

		err = enter_datamode(xdfu_app_datamode_callback,
					xdfu_app_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to enter data write mode: %d", err);
			return err;
		}

		return 0;
	case DFU_OPERATION_APPLY_UPDATE:
		int status = 0;

		err = dfu_target_mcuboot_done(true);
		if (err) {
			LOG_ERR("App firmware update failed: %d", err);
			status = err;
		} else {
			err = dfu_target_mcuboot_schedule_update(0);
			if (err) {
				LOG_ERR("Failed to schedule app firmware update: %d", err);
				status = err;
			}
		}

		urc_send("#XDFU:%u,%u,%d\r\n", DFU_TYPE_APP, DFU_OPERATION_APPLY_UPDATE, status);

		LOG_INF("App firmware update scheduled");

		return 0;
	default:
		LOG_ERR("Invalid operation: %d", op);
		return -EINVAL;
	}

	return 0;
}

static int handle_at_xdfu_delta_mfw(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
				  uint32_t param_count)
{
	int err;
	uint16_t op;

	err = at_parser_num_get(parser, 2, &op);
	if (err) {
		return err;
	}

	switch (op) {
	case DFU_OPERATION_INITIALIZE:
		size_t dfu_size;

		err = at_parser_num_get(parser, 3, &dfu_size);
		if (err) {
			LOG_ERR("Failed to get size: %d", err);
			return err;
		}

		err = dfu_target_modem_delta_init(dfu_size, 0, delta_dfu_evt_handler);
		if (err) {
			LOG_ERR("Failed to initialize delta modem firmware: %d", err);
			return err;
		}

		return 0;
	case DFU_OPERATION_DATA_WRITE:
		if (param_count != 5) {
			LOG_ERR("Invalid number of parameters for data write");
			return -EINVAL;
		}
		err = at_parser_num_get(parser, 3, &xdfu_delta_mfw_datamode_context.addr);
		if (err) {
			LOG_ERR("Failed to get address: %d", err);
			return err;
		}
		err = at_parser_num_get(parser, 4, &xdfu_delta_mfw_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to get length: %d", err);
			return err;
		}

		if (xdfu_delta_mfw_datamode_context.len == 0) {
			LOG_ERR("Length cannot be 0");
			return -EINVAL;
		}

		/* Prepare per-chunk accounting for the datamode callback. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;

		err = enter_datamode(xdfu_delta_mfw_datamode_callback,
					xdfu_delta_mfw_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to enter data write mode: %d", err);
			return err;
		}

		return 0;
	case DFU_OPERATION_APPLY_UPDATE:
		int status = 0;

		err = dfu_target_modem_delta_done(true);
		if (err) {
			LOG_ERR("Delta modem firmware update failed: %d", err);
			status = err;
		} else {
			err = dfu_target_modem_delta_schedule_update(0);
			if (err) {
				LOG_ERR("Failed to schedule delta modem firmware update: %d", err);
				status = err;
			}
		}

		urc_send("#XDFU:%u,%u,%d\r\n",
			 DFU_TYPE_DELTA_MFW, DFU_OPERATION_APPLY_UPDATE, status);

		LOG_INF("Delta modem firmware update scheduled");

		return 0;
	default:
		LOG_ERR("Invalid operation: %d", op);
		return -EINVAL;
	}
	return 0;
}

static int handle_at_xdfu_full_mfw(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
				  uint32_t param_count)
{
	int err;
	uint16_t op;

	err = at_parser_num_get(parser, 2, &op);
	if (err) {
		return err;
	}

	switch (op) {
	case DFU_OPERATION_INITIALIZE:
		err = request_bootloader_mode(true);
		if (err) {
			LOG_ERR("Failed to enable bootloader mode: %d", err);
			return err;
		}

		LOG_PANIC();
		sys_reboot(SYS_REBOOT_COLD);
	case DFU_OPERATION_DATA_WRITE:
		if (param_count != 6) {
			LOG_ERR("Invalid number of parameters for data write");
			return -EINVAL;
		}
		err = at_parser_num_get(parser, 3, &xdfu_full_mfw_datamode_context.type);
		if (err) {
			LOG_ERR("Failed to get type: %d", err);
			return err;
		}

		err = at_parser_num_get(parser, 4, &xdfu_full_mfw_datamode_context.addr);
		if (err) {
			LOG_ERR("Failed to get address: %d", err);
			return err;
		}

		err = at_parser_num_get(parser, 5, &xdfu_full_mfw_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to get length: %d", err);
			return err;
		}

		if (xdfu_full_mfw_datamode_context.len == 0) {
			LOG_ERR("Length cannot be 0");
			return -EINVAL;
		}

		if (xdfu_full_mfw_datamode_context.type != DFU_FULL_MFW_SEGMENT_BOOTLOADER &&
			xdfu_full_mfw_datamode_context.type != DFU_FULL_MFW_SEGMENT_FIRMWARE) {
			LOG_ERR("Invalid segment type: %d",
				xdfu_full_mfw_datamode_context.type);
			return -EINVAL;
		}

		/* Prepare per-chunk accounting for the datamode callback. */
		xdfu_bytes_written = 0;
		xdfu_status = 0;

		err = enter_datamode(xdfu_full_mfw_datamode_callback,
					xdfu_full_mfw_datamode_context.len);
		if (err) {
			LOG_ERR("Failed to enter data write mode: %d", err);
			return err;
		}

		return 0;

	case DFU_OPERATION_APPLY_UPDATE:
		uint16_t type;
		int status = 0;

		if (param_count < 4) {
			LOG_ERR("Invalid number of parameters for bootloader update");
			return -EINVAL;
		}
		err = at_parser_num_get(parser, 3, &type);
		if (err) {
			LOG_ERR("Failed to get type: %d", err);
			return -EINVAL;
		}

		if (type != DFU_FULL_MFW_SEGMENT_BOOTLOADER &&
			type != DFU_FULL_MFW_SEGMENT_FIRMWARE) {
			LOG_ERR("Invalid segment type: %d", type);
			return -EINVAL;
		}

		err = nrf_modem_bootloader_update();
		if (err) {
			LOG_ERR("Failed to update bootloader: %d", err);
			status = err;
		}

		urc_send("#XDFU:%u,%u,%d\r\n",
			 DFU_TYPE_FULL_MFW, DFU_OPERATION_APPLY_UPDATE, status);

		if (status == 0 && type == DFU_FULL_MFW_SEGMENT_FIRMWARE) {
			LOG_INF("Firmware update successful, rebooting...");
			LOG_PANIC();
			sys_reboot(SYS_REBOOT_COLD);
		}

		return 0;

	default:
		LOG_ERR("Invalid operation: %d", op);
		return -EINVAL;
	}
	return 0;
}

SM_AT_CMD_CUSTOM(xdfu, "AT#XDFU", handle_at_xdfu);
static int handle_at_xdfu(enum at_parser_cmd_type cmd_type, struct at_parser *parser,
			  uint32_t param_count)
{
	int err;
	uint16_t dfu_type;

	switch (cmd_type) {
	case AT_PARSER_CMD_TYPE_SET:
		err = at_parser_num_get(parser, 1, &dfu_type);
		if (err < 0) {
			return err;
		}
		switch (dfu_type) {
		case DFU_TYPE_APP:
			return handle_at_xdfu_app(cmd_type, parser, param_count);
		case DFU_TYPE_DELTA_MFW:
			return handle_at_xdfu_delta_mfw(cmd_type, parser, param_count);
		case DFU_TYPE_FULL_MFW:
			return handle_at_xdfu_full_mfw(cmd_type, parser, param_count);
		default:
			return -EINVAL;
		}
	default:
		LOG_ERR("Invalid command type: %d", cmd_type);
		return -EINVAL;
	}

	return 0;
}

int sm_at_dfu_handle_xdfu(char *buf, size_t len, char *at_cmd)
{
	return sm_at_cb_wrapper(buf, len, at_cmd, handle_at_xdfu);
}
