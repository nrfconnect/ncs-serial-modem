...
/* Interceptors registered with AT_CMD_CUSTOM() in sm_at_urc_filter.c. */
extern int sm_urcf_cgerep_callback(char *buf, size_t len, char *at_cmd);
extern int sm_urcf_xtime_callback(char *buf, size_t len, char *at_cmd);
extern int sm_urcf_cfun_callback(char *buf, size_t len, char *at_cmd);
...
static const struct at_dispatch_entry at_dispatch_table[] = {
	...
	{"AT+CGEREP", sm_urcf_cgerep_callback},
	{"AT%XTIME", sm_urcf_xtime_callback},
	{"AT+CFUN=", sm_urcf_cfun_callback},
};
