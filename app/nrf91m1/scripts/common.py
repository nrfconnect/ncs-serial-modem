...
def _resolve_zephyr_base() -> Path:
    ...
    return Path(Manifest.from_file().get_projects(["zephyr"])[0].abspath).resolve()

_ZEPHYR_BASE: Path = _resolve_zephyr_base()

sys.path.insert(0, str(_ZEPHYR_BASE / "scripts"))
import zephyr_module  # noqa: E402

_MODULES_BY_NAME: dict[str, Path] = {
    m.meta["name"]: Path(m.project).resolve()
    for m in zephyr_module.parse_modules(str(_ZEPHYR_BASE))
}

def _module_dir(name: str) -> Path:
    if name not in _MODULES_BY_NAME:
        sys.exit(f"error: zephyr module '{name}' not found (check your west manifest)")
    return _MODULES_BY_NAME[name]

_NRF_MODULE_DIR: Path = _module_dir("nrf")
_MCUBOOT_MODULE_DIR: Path = _module_dir("mcuboot")
