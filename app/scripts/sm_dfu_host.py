...
def dfu_init(dfu_type: int, size: int = None) -> bool:
    """Initialize DFU. Size required for APP/DELTA/MCUBOOT_BL. FULL reboots device."""
    ...
    elif dfu_type == DFU_TYPE_MCUBOOT_BL:
        if size is None:
            return False
        print(f"AT#XDFUINIT={dfu_type},{size}")
        send_command("AT#XDFUINIT", dfu_type, size)
...
def do_dfu_mcuboot_bootloader(filepath: str, retries: int = 3) -> bool:
    ...
    print(
        "MCUboot bootloader DFU: ensure this .bin is the inactive-slot signed image "
        "(s0 vs s1 package from your NCS build output)."
    )
    print(f"MCUboot bootloader DFU: {total_size:,} bytes in {total_chunks} chunks")

    if not dfu_init(DFU_TYPE_MCUBOOT_BL, total_size):
    ...
    for i, (addr, data) in enumerate(chunks, 1):
        ...
        urc = ""
        for _ in range(retries):
            success, urc = dfu_write(DFU_TYPE_MCUBOOT_BL, addr, data)
            if success:
                break
        if not success:
            print(f"\nERROR: Chunk {i} failed after {retries} retries")
        ...
        bytes_sent += len(data)
        cmd = f"AT#XDFUWRITE={DFU_TYPE_MCUBOOT_BL},{addr},{len(data)} -> {urc}"
        print_progress("MCUboot bootloader", i, total_chunks, bytes_sent, total_size, start_time, cmd)

    print()

    if not dfu_apply(DFU_TYPE_MCUBOOT_BL):
        print("ERROR: Apply failed")
        return False

    print("OK: MCUboot bootloader DFU apply complete. Reboot to activate the new MCUboot. Validate the version increase with AT#XBOOTINFO=0.")
    return True
...
def main() -> int:
    ...
    parser.add_argument(
        "--type",
        choices=["application", "modem-delta", "modem-full", "mcuboot-bootloader"],
        help="DFU type",
    )
    ...
    try:
        ...
        if args.ping:
            ...
            elif args.type == "mcuboot-bootloader":
                success = do_dfu_mcuboot_bootloader(str(filepath))
