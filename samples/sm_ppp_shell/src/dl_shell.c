...
/* Sample-local downloader shell: only depends on the public downloader API,
 * so it does not need CONFIG_DOWNLOADER_SHELL (which is disabled here).
 */
#define SM_DL_SHELL_BUF_SIZE 2048

#define SM_DL_SHELL_URL_SIZE (CONFIG_DOWNLOADER_MAX_HOSTNAME_SIZE + \
			      CONFIG_DOWNLOADER_MAX_FILENAME_SIZE + 1)

/** Downloader buffer, allocated by cmd_dc_init() and freed by cmd_dc_deinit(). */
static char *dl_buf;
...
static struct fs_file_t output_file;
static bool output_file_open;
...
static int dl_callback(const struct downloader_evt *event)
{
	...
	case DOWNLOADER_EVT_FRAGMENT:
		if (output_file_open) {
			int ret = fs_write(&output_file, event->fragment.buf, event->fragment.len);

			if (ret < 0 || ret != event->fragment.len) {
				shell_error(shell_instance,
					    "failed to write fragment to file (len %d, ret %d)",
			...
			shell_fprintf(shell_instance, SHELL_NORMAL, "Wrote %d bytes to file\n",
				      event->fragment.len);
		}
...
static int cmd_dc_init(const struct shell *shell, size_t argc, char **argv)
{
	...
	dl_buf = malloc(SM_DL_SHELL_BUF_SIZE);
	if (!dl_buf) {
		shell_error(shell, "Failed to allocate download buffer\n");
		return -ENOMEM;
	}
	dl_cfg.buf = dl_buf;
	dl_cfg.buf_size = SM_DL_SHELL_BUF_SIZE;
...
static int cmd_download_get(const struct shell *shell, size_t argc, char **argv)
{
	...
	url = malloc(SM_DL_SHELL_URL_SIZE);
	if (!url) {
		if (output_file_open) {
			fs_close(&output_file);
			output_file_open = false;
		}
		shell_error(shell, "Failed to allocate url buffer\n");
		return -ENOMEM;
	}

	strncpy(url, dl_url, SM_DL_SHELL_URL_SIZE);
	url[SM_DL_SHELL_URL_SIZE - 1] = '\0';
