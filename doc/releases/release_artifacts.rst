...
The artifacts are zipped containing some of the following files:
...
.. list-table::
   ...
   * - Release artifact
     - Description
   * - ``.hex``
     - The unsigned application binary to be flashed with Programmer or nRF Util.
   * - ``.signed.bin``
     - Signed application only binary to be used in FOTA/DFU.
   * - ``_dfu.zip``
     - Signed application only binary zipped with metadata used by some tools like nRF Cloud for FOTA/DFU.
   * - ``.elf``
     - Application image symbols for debugging purposes.
   * - ``.dts``
     - Devicetree.
   * - ``.config``
     - Application image Kconfig selections.

More details about the build artifacts can be found in the `build output files`_ documentation.
