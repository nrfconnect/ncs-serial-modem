...
A typical Serial Modem application configuration consists of a host processor connected to an nRF91 Series SiP via a UART interface.
...
The DTR and RING signals are the **control signals** used for UART power management, while TX/RX/RTS/CTS are the standard UART signals.


Board-specific pin mapping
**************************

The UART configuration varies depending on the board and whether it's running the Serial Modem application (nRF9151) or a host application (e.g., nRF54L15).

UART configuration is done in the Device Tree as a build time configuration.
...
.. tabs::
   ...
   .. group-tab:: USB UART with PC host
      ...
      * Hardware flow control: Enabled
      ...
      The DTR pin defaults to pull-up with active high level, so the UART is always powered on.
      This pin is shared with Button 1 which can be used to manually toggle the DTR state.
      When the button is pressed, DTR goes low (inactive), powering down the UART.
   ...
   .. group-tab:: external MCU as host
      ...
      * Hardware flow control: Enabled
      ...
      All UART pins are in the P4 connector at the DK board.
   ...
   .. group-tab:: Thingy:91 X with PC host
      ...
      * Hardware flow control: Disabled
      ...
      Hardware flow control is disabled as there is no RTS/CTS pins on the Thingy:91 X interface chip.

      The DTR pin defaults to pull-up with active high level, so the UART is always powered on.
      This pin is shared with Button 0 which can be used to manually toggle the DTR state.
      When the button is pressed, DTR goes low (inactive), powering down the UART.
...
.. tabs::
   ...
   .. group-tab:: PC serial terminal
      ...
      When using a PC serial terminal application (e.g., PuTTY, Tera Term, minicom), the |SM| application should be build to be used with PC host.
      ...
      * AT command terminator: CR
      ...
      For incoming serial output from the modem, both **CR** and **LF** (Line Feed) characters are used. This is the most common configuration for AT command terminals.
   ...
   .. group-tab:: nRF54L15DK
      ...
      UART0 on the P0 connector is the ``uart30`` instance in the device tree and is routed to VCOM0 on the interface chip.
      ...
      **Note:** Disable VCOM0 from `Board Configurator app`_ to release UART30 for Serial Modem.
   ...
   .. group-tab:: Native Simulator
      ...
      The |SM| application should be build to be used with PC host.
...
The |SM| application's UART power state is controlled by the **DTR** (Data Terminal Ready) signal.
...
UART power management is completely independent of the modem power management (CFUN, PSM, eDRX, etc.).
By actively managing the UART power state through DTR control, the nRF9151 SiP achieves ultra-low power consumption of approximately 2 µA when the cellular modem enters PSM (Power Saving Mode) and the UART is powered down.
...
Full network context is maintained, including socket connections, TLS/DTLS sessions, etc.
...
When the UART is powered down and there is incoming data, the |SM| application issues a pulse signal on the **RING** (Ring Indicator) pin to notify the host.
...
The sequence diagram above illustrates the wake-up sequence when the UART is powered down.
...
As the Serial Modem's UART interface power state is controlled by the DTR signal from the host, it is possible to automate the power management using Zephyr's Cellular Modem driver
with Zephyr's CMUX (3GPP TS 27.010) module.
This allows host applications to manage the UART power state automatically. See the `Zephyr CMUX Power Saving`_ documentation for more information.
This configuration is completely done in the host. |SM| does not require any changes as it already follows the DTR signal state for UART power management.

The Zephyr's Cellular Modem driver can be configured to automatically power on and off the UART device based on the data traffic on the CMUX DLCI channels.
