...
* The ``<op>`` parameter must be one of the following values:
  ...
  * ``5`` - Start FOTA for MCUboot second-stage (B1) bootloader update.
...
* The ``<file url>`` parameter is a string.
  ...
  For the MCUboot B1 update, the target image must be the signed image for the **inactive** ``s0``/``s1`` slot.
  ``AT#XB1SLOT`` can be used to query which slot is active.
...
.. note::
   ...
   For the MCUboot B1 update, NSIB (B0) uses monotonic counter values to prevent rollback.
   This means that MCUboot updates for B1 are limited to the number of ``SB_CONFIG_SECURE_BOOT_NUM_VER_COUNTER_SLOTS`` slots.
