...
.. note::
   ...
   * Zephyr cellular modem driver is written with the expectation that the PPP session starts on the DLC channel used for AT commands, which is DLC channel 1 by default.

Example
=======

Zephyr modem driver requires AT channel and PPP channel to be switched so that the PPP starts at DLC channel 1, while the AT channel is moved to DLC channel 2.
This is achieved by starting CMUX with ``AT#XCMUX=2`` before network is attached and starting the PPP.

.. figure:: ../images/ppp-zephyr-sequence.svg
    :alt: PPP session with CMUX showing Zephyr host control.
    :align: center

    Zephyr modem driver does extra DLC channel switching when starting the PPP session.
