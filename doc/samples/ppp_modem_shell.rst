.. _ppp_modem_shell_sample:

PPP Modem Shell sample
######################
...
The PPP Modem Shell sample demonstrates how to use the nRF91 Series SiP as a Zephyr-compatible cellular modem through a PPP (Point-to-Point Protocol) interface.
...
For more information about using the nRF91 Series as a Zephyr-compatible modem, see :ref:`sm_as_zephyr_modem`.
...
.. note::
   You must disable the VCOM0 on the nRF54L15 DK to release the UART GPIO pins for communication with the Serial Modem.
...
The modem will begin the registration process.
...
After the interface is up, you can test basic connectivity:
...
To run a TCP upload test:

.. code-block:: console

   uart:~$ zperf tcp upload <server-ip> <port> <duration> <packet-size>
...
* Networking stack (:kconfig:option:`CONFIG_NETWORKING`)
* PPP L2 layer (:kconfig:option:`CONFIG_NET_L2_PPP`)
* Cellular modem driver (:kconfig:option:`CONFIG_MODEM_CELLULAR`)
* Network shell (:kconfig:option:`CONFIG_NET_SHELL`)
* Zperf (:kconfig:option:`CONFIG_NET_ZPERF`)
* Power management (:kconfig:option:`CONFIG_PM_DEVICE`)
