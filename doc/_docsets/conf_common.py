...
% upstream's catcode setup untouched, and \begingroup keeps \fboxsep out of the
% surrounding text, since the hook runs before the environment opens its own
% group. Sphinx colours nothing in a sphinxalltt, so pre_TeXcolor does not
...
%
% \begingroup alone does not hold the grey, because framed.sty collects the
% panel and re-emits it, which loses the colour pop that closes the group and
% leaves the body text after the block grey. \color@begingroup and
% \color@endgroup balance the pushes and pops across that rebuilding, and are
% what Sphinx itself uses around every framed panel it draws, calling it an
% upstream framed.sty bug. The colour has to be set inside that pair, and the
% pair inside the panel. Both commands are internal, hence \makeatletter.
...
def docset_html_context(docset: str) -> dict:
    """Return the ``html_context`` additions for a docset.

    Exposes the per-docset PDF filename so that Jinja2 templates can link to
    the correct PDF without hardcoding a name that is only valid for one
    docset.
