...
#define MODEM_READY_TIMEOUT     K_SECONDS(120)
...
static int pipe_write_blocking(struct modem_pipe *pipe, const uint8_t *buf, size_t size)
{
	...
	k_timepoint_t t = sys_timepoint_calc(XDFU_DATA_WRITE_TIMEOUT);
	...
	while (size > 0) {
		...
		if (sys_timepoint_expired(t)) {
			LOG_ERR("Timeout while writing to pipe");
			return -ETIMEDOUT;
		}
	}
...
static int xdfu_fmfu_from_file(const char *file)
{
	...
	for (int i = 0; i < segments.Segments_Segment_m_count; i++) {
		...
		/* Modem flash is touched from here on, so a failure leaves it unusable. */
		modem_unusable = true;
		...
		if (i == 0) {
			...
			if (ret < 0) {
				LOG_ERR("Failed to apply bootloader: %d", ret);
				goto out;
			}
		}
	...
	ret = xdfu_apply(SM_XDFU_TYPE_FULL_MFW);
	if (ret < 0) {
		LOG_ERR("Failed to apply firmware: %d", ret);
		goto out;
	}
out:
	if (ret < 0 && modem_unusable) {
		LOG_ERR("The modem is unusable. Retry the full DFU process");
	}
	free(meta_buf);
	return ret;
}
...
int sm_xdfu_run(const struct device *modem, enum sm_xdfu_image_type type, const char *file)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed get file status: %d", ret);
		return ret;
	}
...
cleanup:
	...
	if (type != SM_XDFU_TYPE_FULL_MFW || ret < 0) {
		k_event_clear(&xdfu_event, BIT(READY) | BIT(FAILURE));
	...
	if (!wait_for_dfu_event(READY, MODEM_READY_TIMEOUT)) {
